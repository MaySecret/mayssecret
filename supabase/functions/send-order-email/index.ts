...
const STATUS_COPY: Record<string, { subject: (c: string) => string; heading: string; body: string }> = {
  placed: {
    subject: (c) => `Your May's Secret order ${c} is received`,
    heading: "Thank you for your order",
    body: "We've received your order and are awaiting payment confirmation. You'll hear from us again the moment payment clears.",
  },
  paid: {
    subject: (c) => `Payment confirmed — order ${c}`,
    heading: "Payment confirmed",
    body: "Your payment has been received. Your fragrance is now being prepared for shipment.",
  },
  shipped: {
    subject: (c) => `Your May's Secret order ${c} is on its way`,
    heading: "Your order has shipped",
    body: "Your fragrance is on its way to you. Keep an eye on your inbox — we'll let you know once it arrives.",
  },
  delivered: {
    subject: (c) => `Your May's Secret order ${c} has arrived`,
...
function renderEmail(opts: {
  ...
  const itemsHtml = opts.items
    .map(
      (it) => `
      <tr>
        <td style="padding:12px 0;border-bottom:1px solid #f5dcd6;color:#3a1820;font-family:Georgia,serif;">
          ${it.product_name} <span style="color:#8a5260;">— ${it.variant_size} × ${it.quantity}</span>
        </td>
        <td style="padding:12px 0;border-bottom:1px solid #f5dcd6;text-align:right;color:#3a1820;font-family:Georgia,serif;">
...
<body style="margin:0;padding:0;background:#ffffff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#ffffff;padding:40px 20px;">
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #f5dcd6;">
        <tr><td style="background:#3a1820;padding:32px;text-align:center;">
          <h1 style="margin:0;font-family:Georgia,serif;font-weight:400;font-size:32px;color:#fcc6be;letter-spacing:-0.01em;">May's Secret</h1>
        </td></tr>
        ...
        <tr><td style="padding:0 32px 24px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #f5dcd6;border-bottom:1px solid #f5dcd6;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;">
            ${itemsHtml}
            <tr>
              <td style="padding:16px 0 4px;color:#8a5260;font-size:13px;">Subtotal</td>
              <td style="padding:16px 0 4px;text-align:right;color:#3a1820;font-size:13px;">${fmt(opts.subtotal)}</td>
            </tr>
            <tr>
              <td style="padding:4px 0;color:#8a5260;font-size:13px;">Shipping</td>
              <td style="padding:4px 0;text-align:right;color:#3a1820;font-size:13px;">${fmt(opts.shipping)}</td>
            </tr>
            <tr>
              <td style="padding:14px 0 0;font-family:Georgia,serif;font-size:20px;color:#3a1820;border-top:1px solid #f5dcd6;">Total</td>
              <td style="padding:14px 0 0;text-align:right;font-family:Georgia,serif;font-size:20px;color:#3a1820;border-top:1px solid #f5dcd6;">${fmt(opts.total)}</td>
            </tr>
          </table>
        </td></tr>
        <tr><td style="padding:32px;background:#fff1ee;text-align:center;">
          <p style="margin:0;font-size:12px;color:#8a5260;line-height:1.6;">
            May's Secret · Lagos, Nigeria<br>
...
async function sendWithRetry(payload: any, label: string, attempts = 3): Promise<{ ok: boolean; error?: string }> {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (res.ok) {
        console.log(`[send-order-email] ${label} sent ✓ (attempt ${i + 1})`);
        return { ok: true };
      }
      const txt = await res.text().catch(() => "");
      console.error(`[send-order-email] ${label} attempt ${i + 1} ${res.status}:`, txt);
      if (res.status >= 400 && res.status < 500 && res.status !== 429) {
        return { ok: false, error: `${res.status} ${txt.slice(0, 200)}` };
      }
    } catch (e) {
      console.error(`[send-order-email] ${label} attempt ${i + 1} threw:`, e);
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { status, orderCode, customerName, customerEmail, subtotal, shipping, total, items } = data ?? {};

    if (
      !status || !orderCode || !customerName || !customerEmail ||
      typeof total !== "number" || typeof subtotal !== "number" || typeof shipping !== "number" ||
    ...
    const adminHtml = renderEmail({
      heading: status === "paid" ? `New paid order: ${orderCode}` : `Order ${orderCode} → ${status}`,
    ...
    const customerSend = sendWithRetry({
      from: FROM,
      to: [customerEmail],
    ...
    const sends = [customerSend];

    if (status === "paid" || status === "placed") {
      sends.push(sendWithRetry({
        from: FROM,
        to: [ADMIN_EMAIL],
        subject: `[May's Secret] ${status === "paid" ? "Paid" : "New"} order ${orderCode} — ${customerName}`,
        html: adminHtml,
      }, "admin"));
    }
